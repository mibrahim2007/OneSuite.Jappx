...
type BillFormProps = {
  ...
  taxRates: TaxRate[];
  initialData?: Bill & { lines: BillLine[] };
};
...
export function BillForm({
  ...
  const form = useForm<SaveBillValues>({
    resolver: zodResolver(saveBillSchema),
    defaultValues: isEdit
      ? {
          ...
          payableAccountId: initialData.payableAccountId,
  ...
  const watchedDate = form.watch("billDate");

  // Auto-select period matching bill date
  useEffect(() => {
  ...
  // Grouped accounts for expense lines (exclude bank accounts)
  const expenseGroupedAccounts = useMemo(() => {
  ...
  // Liability accounts only for the AP payable account
  ...
  function onSubmit(values: SaveBillValues) {
  ...
  return (
    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 max-w-5xl">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <div className="space-y-1.5">
          <Label htmlFor="reference">Reference (Vendor Invoice #)</Label>
          <Input
            id="reference"
        ...
        <div className="space-y-1.5">
          <Label htmlFor="payableAccountId">AP Account (Payable)</Label>
          <select
            id="payableAccountId"
            className={SELECT_CLASS}
            {...form.register("payableAccountId")}
          >
            ...
            >
              <div>
                ...
                >
                  ...
                  {Array.from(expenseGroupedAccounts.entries()).map(
                    ([type, accts]) =>
                      accts.length === 0 ? null : (
                        <optgroup
                          key={type}
      ...
      <div className="flex gap-3">
        <Button type="submit" disabled={isSaving || !hasOpenPeriods}>
          {isSaving ? "Saving…" : isEdit ? "Update Bill" : "Save Draft"}
        </Button>
        <Button
          type="button"
          variant="outline"
